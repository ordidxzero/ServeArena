...
# 49152
server_cmd = CommandTemplate("vllm serve meta-llama/Llama-3.2-1B-Instruct --dtype float16 --max-model-len 8192 --max-num-seqs 64 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.1946 --no-enable-chunked-prefill")
benchmark_cmd = CommandTemplate("vllm bench serve --backend vllm --model meta-llama/Llama-3.2-1B-Instruct --endpoint /v1/completions --ignore-eos --dataset-name random --random-input-len {input_len} --random-output-len {output_len} --num-prompts 1024 --disable-tqdm --percentile-metrics ttft,tpot,itl,e2el --save-result --save-detailed --result-dir './outputs/benchmark' --result-filename {benchmark_detailed_log}")
